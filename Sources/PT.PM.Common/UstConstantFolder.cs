...
namespace PT.PM.Common
{
    public class UstConstantFolder : ILoggable
    {
        ...
        private readonly Dictionary<int, FoldResult> foldedResults = new Dictionary<int, FoldResult>();
        ...
        public bool TryGetOrFold(Ust ust, out FoldResult result)
        {
            ...
            lock (ust)
            {
                if (foldedResults.TryGetValue(ust.TextSpan.GetHashCode(), out result))
                {
                    return result != null;
                }
        ...
        {
            if (ust != null && foldedResults.TryGetValue(ust.TextSpan.GetHashCode(), out FoldResult result))
            {
                return result;
            }
        ...
        private void NormalizeAndAdd(Ust ust, ref FoldResult result)
        {
            ...
            foldedResults[ust.TextSpan.GetHashCode()] = result;
        }
    }
}